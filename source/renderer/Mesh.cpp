...
Mesh& Mesh::operator=(Mesh&& other) noexcept
{
	if (this != &other)
	{
		...
		m_localMaterialIndex = other.m_localMaterialIndex;
...
void Mesh::Upload(RenderContext& context, const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices, const std::string& name)
{
	...
	m_vertexBuffer = std::make_unique<StructuredBuffer>(
		context, m_vertexCount, static_cast<uint32_t>(sizeof(Vertex)), D3D12_RESOURCE_FLAG_NONE, D3D12_HEAP_TYPE_DEFAULT, (name + "_VB").c_str());
